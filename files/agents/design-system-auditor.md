...
- **Runtime contrast verification** -> `contrast-master` (checks rendered UI, not tokens; also installed)
- **Full web audit**, **mobile touch target validation**, **WCAG criterion questions** -> not installed. This agent was vendored standalone from `Community-Access/accessibility-agents`; the `accessibility-lead`, `mobile-accessibility`, and `wcag-guide` agents it hands off to in the upstream repo were not brought over. Handle these directly or note the gap to the user.
